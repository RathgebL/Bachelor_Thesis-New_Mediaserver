...
\begin{itemize}
  \item \textbf{Datenquelle:} Herkunft und Aufbereitung der Mediendaten.
  \item \textbf{Datenstruktur:} Aufbau und Formatierung der Daten bisher und im neuen Zielsystem.
  \item \textbf{Datenübertragung:} Mechanismen zum Import und zur Synchronisation neuer Dateien.
  \item \textbf{Datenverwaltung:} Pflege, Sicherung und Backup der Bestände.
\end{itemize}
...
sudo nano /etc/systemd/system/process_incoming.service
    ...
    Inhalt der Datei:
    \begin{minted}[breaklines, linenos]{ini}
# ===============================================
...
sudo systemctl list-timers --all | grep process_incoming
  ...
  fehlt noch ein Upload-Ordner aus dem automatisch Dateien an den Server geschickt werden.
  ...
  \begin{enumerate}
    ...
    \begin{minted}[breaklines, linenos]{bash}
mkdir /Desktop/_upload-to-server
mkdir /Desktop/Scripts
nano /Desktop/Scripts/upload_media.sh
    \end{minted}
...
ping mediaserver.local
  ...
  \subsubsection*{Probleme mit launchd (automatischer Datentransfer zum Server)}
  Wenn für den SSH-Key ein Passwort dei der Erstellung angelegt wurde, kann es zu Problemen bei der Automatisierung des Datentrasfers mit launchd kommen.
  Dies lässt sich aber einfach lösen in dem man das Passwort zur Keychain hinzufügt.
  
  \begin{minted}[breaklines, linenos]{bash}
ssh-add --apple-use-keychain ~/.ssh/id_ed25519 # Key gegebenfalls anpassen
  \end{minted}

  Nun das Passwort angeben. Danach testen:
  \begin{minted}[breaklines, linenos]{bash}
ssh medienserver@Server-IP
  \end{minted}

  Es sollte nicht mehr nach dem Passwort gefragt werden.

  \subsubsection*{Probleme mit systemd (automatische Einsortierung der Medien auf Serverseite)}
  Damit das Skript \texttt{process\_incoming.sh} ordnungsgemäß ausgeführt werden kann, 
  müssen die Zugriffsrechte auf alle beteiligten Verzeichnisse korrekt gesetzt werden. 
  Insbesondere benötigt der in der Service-Datei angegebene Benutzer \texttt{medienserver} Schreibrechte auf die Logdateien sowie Lese- 
  und Schreibrechte auf die Upload- und Zielordner.
...
sudo chmod 664 /srv/logs/process.log
  ...
  Das Vergeben der richtigen Besitzrechte ist insbesondere deshalb erforderlich, weil der \texttt{systemd}-Dienst standardmäßig mit dem Benutzer \texttt{medienserver} läuft und 
