...
sudo mkdir -p /srv/navidrome/data
    ...
    Außerdem muss die Festplatte mit den Musikdateien (z.\,B. eine interne Festplatte) eingebunden werden.
    
    \begin{minted}[breaklines, linenos]{bash}
# Platte montieren und Gerät ermitteln (z.B. sdb)
lsblk
...
server {
    ...
    location / {
        proxy_pass         http://193.197.85.23:4533; # hier Server-IP eintragen
...
Diese Schritte geschehen in enger zusammenarbeit mit den IT-Mitarbeitern der Hochschule.
  ...
  Der Server ist per \ac{LAN} angebunden und ist über seine \ac{IP}-Adresse im Netzwerk erreichbar. 
  Damit der Zugriff zuverlässig funktioniert, sind mehrere Punkte zu beachten:

  \begin{itemize}
    \item \textbf{Sicherheit:} bisher nur provisorisch gesetzte Passwörter sollten, falls nötig, zu starken Passwörtern geändert und in einem Passwortmanager gespeichert werden, bevor der Server ans Netz geht.
    \item \textbf{Firewall-Konfiguration:} Die Firewall der HfM muss so eingestellt sein, dass die nötigten Ports geöffnet sind.
    \item \textbf{Subnetze und \ac{VLAN}s:} Im Hochschulnetzwerk müssen die relevanten Subnetze für den Zugriff freigeschaltet werden. 
          Über die Segmentierung mittels VLANs lässt sich steuern, in welchen Netzwerkzonen Clients den Server erreichen dürfen.
    \item \textbf{\ac{DNS}-Eintrag:} Damit Nutzer nicht auf die \ac{IP}-Adresse angewiesen sind, sollte ein aussagekräftiger \ac{DNS}-Hostname für den Server angelegt werden.
\end{itemize}

  Durch diese Maßnahmen ist gewährleistet, dass der Medienserver nahtlos in die bestehende IT-Infrastruktur integriert wird, sicher betrieben werden kann und von den vorgesehenen Nutzergruppen erreichbar ist.
