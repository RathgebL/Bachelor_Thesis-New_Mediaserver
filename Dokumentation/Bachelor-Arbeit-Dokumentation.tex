...
\section{Zielsetzung}
Als bei mir die Idee, einen neuen Medienserver in der Bibliothek der \ac{HfM} Karlsruhe aufzubauen, aufkam, war meine Vorstellung, eine moderne, flexible, benutzerfreundliche 
und nachhaltige Lösung selbst zu programmieren, die es ermöglicht, die digitalisierten CDs zugänglich zu machen.
Im Gespräch mit dem Bibliotheksleiter Marc Weisser erfuhr ich, dass der alte, nicht mehr betriebene Medienserver damals als Masterarbeit von Frederik Schroff konzipiert und umgesetzt wurde.
Damit war klar, einen komplett neuen Medienserver, mit eigenst programmierter Nutzeroberfläche, 
und Datenbank aufzubauen, wäre ein sehr umfangreiches Projekt, das den Rahmen einer Bachelorarbeit schnell sprengen würde.
Somit informierte ich mich über bestehende, quelloffene Medienserver-Software, die meinen Anforderungen möglichst nahekommt, um einen Kompromiss für die Umsetzbarkeit zu finden. 
Im folgen soll es nun darum gehen, welche Ziele ich mir im hinblick auf Finktionalität, Benutzerfreundlichkeit, Wartbarkeit, Sicherheit und rechtliche Rahmenbedingungen gesetzt habe.
  
Am Ende wird im Fazit überprüft, inwieweit diese Ziele erreicht wurden, welche Herausforderungen es gab 
und welche Verbesserungen oder Erweiterungen in Zukunft noch möglich sind.

  \subsection{Funktionalität}
  Primäre Anforderungen im Bereich der Funktionalität sind flüssiges Streaming der digitalisierten CDs und die Option die entsprechenden Booklets abzurufen zu können.
  Dies soll über eine Weboberfläche erfolgen.

  \subsection{Benutzerfreundlichkeit}
  Um das Angebot des neuen Medienservers für Studenten und Dozenten möglichst attraktiv zu machen, ist es wichtig, dass die Weboberfläche möglichst einfach zu bedienen ist.
  Dafür sollte deren Aufbau, Design und Funktionalität möglichst nah an bekannten Plattformen, wie z.B. Spotify, liegen.
  Erweiterte Funktionalitäten, wie das Anlegen eines personalisierten Benutzeracoounts, um z.B. eigene Playlisten anlegen zu können, sollten über eine simple Registrierung per E-Mail Adresse und dem Setzen eines Passworts zugänglich sein.
  Natürlich sollte die Nutzung aber auch ohne Anmeldung funktionieren. Dafür soll es einen Bibliotheks-Account gebn, der frei genutzt werden kann.
  Der Zugriff auf den Server sollte primär von einem Rechner in der Bibliothek (ähnlich wie der Naxos-Rechner) möglich sein. 
  Eventuell sogar generell von Rechnern, die mit dem lokalen Neztwerkverbunden sind.
