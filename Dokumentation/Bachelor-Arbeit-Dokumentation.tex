...
Damit soll der Aufbau des Medienservers in der Bibliothek nachvollziehbar und reproduzierbar gemacht werden.
  ...
  \begin{enumerate}
    ...
    \begin{minted}[breaklines, breaksymbolleft=, breaksymbolright=, linenos]{bash}
...
sudo nano docker-compose.yml
    ...
    \begin{minted}[breaklines, breaksymbolleft=, breaksymbolright=, linenos]{yaml}
...
services:
  navidrome:
    ...
    \begin{minted}[breaklines, breaksymbolleft=, breaksymbolright=, linenos]{toml}
# -----------------------------
# Pfade
# -----------------------------
MusicFolder = "/music"
...
wie der automatisierte Datentransfer, die Strukturierung und die Sicherung der Mediendaten umgesetzt werden können.  
  ...
  \begin{minted}[breaklines, breaksymbolleft=, breaksymbolright=, linenos]{text}
audio1/
...
audio1/
  ...
  Das Skript \texttt{jpg2pdf.py} übernimmt die automatisierte Aufbereitung und Vereinheitlichung der digitalisierten CD-Booklets. 
  Da die Scans der Begleithefte im Digitalisierungsprozess zunächst als einzelne \texttt{.jpg}-Dateien vorliegen, 
  ist eine strukturierte Zusammenführung zu vollständigen PDF-Dokumenten erforderlich. 
  Diese PDFs werden anschließend gemeinsam mit den konvertierten Audiodateien auf dem Medienserver bereitgestellt 
  und bilden die Grundlage für den Zugriff auf die digitalen Booklets im Web-Browser.

  Das Skript durchsucht rekursiv alle Unterverzeichnisse des gewählten Eingabeordners nach \texttt{booklet}-Ordnern 
  und erkennt automatisch die enthaltenen JPG-Dateien. 
  Die einzelnen Seiten werden in logischer Reihenfolge (beginnend mit \texttt{booklet-b.jpg}, gefolgt von \texttt{booklet.jpg} und etwaigen nummerierten Varianten) 
  zu einem zusammenhängenden PDF-Dokument kombiniert. 
  Der Name des erzeugten PDFs wird dabei automatisch aus der Verzeichnisstruktur abgeleitet, 
  sodass eine konsistente und menschenlesbare Benennung nach dem Schema \texttt{<Komponist>-<Medientitel>.pdf} entsteht.

  Besonders wichtig ist dabei, dass die Benennung exakt nach demselben Schema wie im Metadatenskript erfolgt. 
  Nur dadurch kann gewährleistet werden, dass die in den FLAC-Dateien hinterlegten Booklet-URLs 
  mit den tatsächlich erzeugten PDF-Dateien übereinstimmen und die Verknüpfung über die Web-Oberfläche korrekt funktioniert. 
  Um dies sicherzustellen, verwendet das Skript dieselben Funktionen zur Text- und Unicode-Normalisierung wie das Metadatenskript.

  Der Booklet-Generator arbeitet ebenfalls parallelisiert über mehrere CPU-Kerne 
  und zeigt den Fortschritt der Verarbeitung im Terminal an. 
  Bereits vorhandene PDF-Dateien werden erkannt und übersprungen, um unbeabsichtigtes Überschreiben zu vermeiden. 
  Fehlerhafte oder unvollständige Bilddateien werden mit einer Warnung übersprungen, 
  ohne den Ablauf der Verarbeitung zu unterbrechen.  
  Durch diese robuste Verarbeitung kann das Skript auch auf große Mengen digitalisierter Medienordner angewendet werden.

  Mit diesem Werkzeug wird sichergestellt, dass alle digitalen Booklets in einem einheitlichen, 
  archivtauglichen und serverkompatiblen Format vorliegen.  
  Der vollständige Quellcode des Skripts ist sowohl im GitHub-Repository dieser Bachelorarbeit als auch im Anhang enthalten 
  und kann bei Bedarf leicht erweitert oder in andere Verarbeitungsschritte integriert werden.

  \subsubsection*{Externe und weitere Quellen}
  Neue Mediendaten können mithilfe der zuvor beschriebenen Skripte jederzeit in das System integriert werden. 
  Die beiden Werkzeuge zur Metadatenanreicherung und Booklet-Aufbereitung sind so konzipiert, 
  dass sie auch mit extern erzeugten Digitalisaten oder bereits vorhandenen Musikdateien arbeiten können. 
  Das zugrunde liegende Datenformat kann bei Bedarf an die Struktur des Medienservers angepasst oder, 
  sofern kompatibel, direkt übernommen werden (siehe hierzu nach unterstützte Datenformate für Navidrome).

  Beispielsweise können MP3-Dateien ohne vorherige Konvertierung direkt auf den Server hochgeladen werden. 
  Sind in diesen Dateien bereits Metadaten enthalten, werden diese von Navidrome automatisch übernommen und verarbeitet. 
  Alternativ können vorhandene Audiodateien mit Hilfe der beschriebenen Konvertierungs- und Metadaten-Skripte 
  vereinheitlicht und mit zusätzlichen Informationen angereichert werden, um eine konsistente Darstellung im System sicherzustellen.

  Darüber hinaus besteht die Möglichkeit, zukünftig auch spezielle Audiodatenbestände einzubinden, 
  wie etwa einzigartige Aufnamen, die das Institut für Musikinformatik der Hochschule 
  vom ZKM Karlsruhe erworben hat. 
  Diese Daten konnten im Rahmen der vorliegenden Arbeit aus Zeitgründen noch nicht berücksichtigt werden, 
  sollen jedoch perspektivisch in das System integriert werden, 
  um das digitale Angebot des Medienservers weiter zu vervollständigen.
...
sudo nano /etc/systemd/system/process_incoming.service
    ...
    Inhalt der Datei:
    \begin{minted}[breaklines, breaksymbolleft=, breaksymbolright=, linenos]{ini}
# ========================================================
...
nano de.hfm.upload_media.plist
    ...
    \begin{minted}[breaklines, breaksymbolleft=, breaksymbolright=, linenos]{XML}
...
<dict>
    ...
    \item Launchd-Job laden und testen:
    \begin{minted}[breaklines, breaksymbolleft=, breaksymbolright=, linenos]{bash}
...
Eine zentrale Voraussetzung für den langfristigen Betrieb des Medienservers ist eine strukturierte und nachhaltige Verwaltung der digitalen Bestände. 
Hierzu zählen sowohl die kontinuierliche Pflege der vorhandenen Metadaten als auch eine verlässliche Backup-Strategie für die Sicherung der Audiodateien und zugehörigen Booklets.

  \subsubsection*{Datenpflege}
  Die Pflege der Datenbestände umfasst sowohl inhaltliche als auch technische Aspekte. 
  Auf inhaltlicher Ebene müssen neu digitalisierte oder nachträglich korrigierte Medien regelmäßig in das System eingepflegt werden. 
  Hierbei ist insbesondere auf eine konsistente Benennung und vollständige Metadatenstruktur zu achten, 
  damit die Such- und Filterfunktionen des Medienservers korrekt arbeiten. 
  Die im Rahmen dieser Arbeit entwickelten Skripte zur Metadatenanreicherung und Booklet-Aufbereitung 
  unterstützen diesen Prozess, indem sie eine einheitliche Datenstruktur sicherstellen und somit die Pflege der Bestände erheblich vereinfachen.

  Darüber hinaus sollte in regelmäßigen Abständen eine Kontrolle der Metadatenqualität erfolgen, 
  um mögliche Inkonsistenzen oder fehlerhafte Zuordnungen frühzeitig zu erkennen. 
  Hierfür kann die bestehende Ordnerstruktur automatisiert überprüft oder über die Web-Oberfläche 
  ein Abgleich der gespeicherten Daten vorgenommen werden.

  \subsubsection*{Backup-Strategie}
  Für die Sicherung der Mediendaten wurde eine neue 8\,TB-HDD beschafft, 
  die als zentrales Backup-Medium für alle FLAC-Dateien und die dazugehörigen PDF-Booklets dient. 
  Die Sicherung erfolgt regelmäßig und soll gewährleisten, 
  dass auch im Falle eines Hardwareausfalls oder versehentlichen Löschens keine Datenverluste auftreten. 
  Das Backup wird dabei als vollständige Kopie des aktuellen Serverbestands geführt, 
  wobei sichergestellt ist, dass sowohl die Dateistruktur als auch alle Metadaten erhalten bleiben.

  Da sämtliche Daten des Medienservers aus den originalen Digitalisaten jederzeit erneut erzeugt werden können, 
  stellt das Backup in erster Linie eine operative Redundanz dar, 
  die eine schnelle Wiederherstellung des Systems im Störungsfall ermöglicht. 
  Langfristig kann darüber hinaus geprüft werden, ob zusätzlich eine zweite, 
  externe Sicherung (z.\,B. auf einem separaten Speichersystem oder Cloud-Speicher) eingerichtet werden sollte, 
  um den Schutz der Archivdaten weiter zu erhöhen.
